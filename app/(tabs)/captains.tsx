...
export default function CaptainsScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const filtered = allPlayers.filter((p: Player) => p.isCaptain && p.YoB <= new Date().getFullYear() - 34);
      (filtered as Player[]).sort((a: Player, b: Player) => a.MinutesPlayed - b.MinutesPlayed);
      setCaptains(filtered);
      const favs = await AsyncStorage.getItem(FAVORITES_KEY);
      setFavorites(favs ? JSON.parse(favs) : []);
    } catch (error) {
      console.error('Error loading captains or favorites:', error);
    }
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const updated = favorites.filter((id) => id !== playerId);
  ...
  return (
    <View>
      <Text style={styles.header}>Captains</Text>
      {captains.length > 0 && (
